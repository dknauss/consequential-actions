...
- Verified `wp_parse_auth_cookie()` (`wp-includes/pluggable.php:1013`) only splits the cookie on `|` and counts four parts — no HMAC check, no expiry check — which is why `wp_get_session_token()` alone cannot be trusted and `WP_Session_Tokens::verify()` is used instead.
- Ran PHP syntax checks on first-party PHP files: passed.
- Ran `composer validate --strict`: passed.
- Ran the unit suite: passed, 49 tests / 156 assertions.
- Ran the integration suite locally against MySQL 8 + the WordPress test library: passed, 26 tests / 62 assertions across `FormGateTest`, `RestGateTest`, `BulkGateTest`, `WindowBindingTest`, `SmokeTest`. Also green in CI (integration runs on PHP 8.2 and 8.3 against WP 6.4 and latest; the unit matrix is the one that spans PHP 7.4–8.3).
...
**Location:** `consequential-actions.php:906-917` (`pending_key()` / `reauthed_key()`)
...
Two structural weaknesses are now closed. The sudo window is bound to the login session that opened it, so a cloned cookie, a second device, and any bearer credential are excluded rather than elevated by the legitimate user's confirmation. And REST accepts no password at all, which removes the guessing channel for every credential class rather than special-casing one. A live gate bypass — a case-sensitive route test against a case-insensitive dispatcher — was found and fixed in both the route and the target segment.

It is still **not safe to run on a production site**, and the reason is narrower than before: the wp-admin confirm field remains un-throttled (CA-2), and reaching it requires exactly what this demo dramatises — a live admin session. CA-6 (Application-Password issuance) remains ungated, so a leaked credential retains a persistence path that survives the password change the demo protects. Land the PR #4 bound and gate the app-passwords route, and the remaining gap is a documented scope boundary rather than a hole.
