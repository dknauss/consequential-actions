#!/usr/bin/env bash
# Install the WordPress test suite for integration tests.
# Canonical WP-CLI scaffold script (trimmed). Usage:
...
if [[ "$WP_VERSION" =~ ^[0-9]+\.[0-9]+\-(beta|RC)[0-9]+$ ]]; then
	...
	# Latest: read the current stable version from the API.
	...
	if [[ -z "$LATEST_VERSION" ]]; then
		echo "Could not determine latest WordPress version"; exit 1
	fi
...
install_test_suite() {
	...
	if [[ $(uname -s) == 'Darwin' ]]; then local ioption='-i .bak'; else local ioption='-i'; fi
	...
	if [ ! -f "$WP_TESTS_DIR"/wp-tests-config.php ]; then
		...
		local WP_CORE_DIR_ESC=$(echo "$WP_CORE_DIR" | sed "s:/:\\\/:g")
...
install_test_suite
create_db
echo "WordPress test suite installed: WP_TESTS_DIR=$WP_TESTS_DIR WP_CORE_DIR=$WP_CORE_DIR"
