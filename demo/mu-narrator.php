	...
	function () {
		...
		if ( 'profile' === $screen->id ) {
			...
			echo '<div class="notice notice-info"><p><strong>Prove it over the REST API.</strong> '
				...
				. 'because the same rule runs on the REST route.</p>'
				. '<p><strong>Now try to talk your way past it.</strong> Add '
				. '<code>ca_confirm_password: \'password\'</code> to the JSON body and run it again. '
				. 'It is <em>still</em> <code>403</code>. REST does not accept a password as proof of '
				. 'intent, on purpose: a password field on an API route is an unthrottled guessing '
				. 'oracle, and any leaked token could mine it. Proof of intent belongs to an '
				. 'interactive session: on a normal install, confirming in wp-admin opens a short '
				. 'window bound to <em>that browser&rsquo;s</em> login session, and a REST call from '
				. 'the same browser rides it. A cloned cookie, a second device, or an Application '
				. 'Password cannot borrow that window &mdash; the key is derived from the session '
				. 'token. <em>(This demo pins the window to 0 so every action re-challenges and you '
				. 'can repeat the walkthrough, so there is no window to observe here.)</em></p>'
				. '<p>(The same guard on a second surface. A real core primitive would sit at the '
